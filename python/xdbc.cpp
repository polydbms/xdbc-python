    ...
    while (c.hasNext(thread_num)) {
        ...
        if (curBuffWithId.id >= 0) {
            ...
            if (curBuffWithId.iformat == 2) {
                ...
                size_t processedTuples = 0; // Start from the beginning of the buffer

                while (processedTuples < curBuffWithId.totalTuples) {
                    if (cnt >= end_idx) {
                        spdlog::get("pyXCLIENT")->info("Thread {0}, requesting slice", thread_num);
                        auto p = queue->pop();
                        cnt = p.startOffset;
                        end_idx = p.endOffset;
                        spdlog::get("pyXCLIENT")->info("Thread {0}, got slice [{1},{2}]", thread_num, cnt, end_idx);
                    ...
                    // Calculate how many tuples to process in this iteration
                    size_t tuplesToProcess = std::min(
                        curBuffWithId.totalTuples - processedTuples, // Remaining tuples in the buffer
                        static_cast<size_t>(end_idx - cnt)          // Remaining tuples in the current slice
                    );
                    ...
                    for (size_t j = 0; j < schemaSize; ++j) {
                        ...
                        // Calculate the pointer for the current attribute and tuple
                        char *data_ptr = reinterpret_cast<char *>(pointers[j]) + processedTuples * attr_sizes[j];
                    ...
                    // Push the processed range to the serialization queue
                    Partition serTask{cnt, cnt + tuplesToProcess};
                    serqueue->push(serTask);

                    // Update counters
                    processedTuples += tuplesToProcess; // Increment processed tuples
                    cnt += tuplesToProcess;            // Increment global count
                    totalCnt += tuplesToProcess;       // Increment total count
                }
        ...
        env.pts->push(xdbc::ProfilingTimestamps{std::chrono::high_resolution_clock::now(), thread_num, "write", "push"});
...
py::list load(py::dict pyEnv) {
    ...
    spdlog::get("pyXCLIENT")->info("Created empty string arrays");
    ...
    for (auto &column : char_columns) {
        ...
        // Append the filled array to the result
        result.append(str_array);

        // Clear and shrink the column to free memory
        column.clear();
        column.shrink_to_fit();
